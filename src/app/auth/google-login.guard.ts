...
import { AuthState, cognitoTokens, loggedInUser } from './+state/auth.reducer';
import { LoggedInUser, UserRole } from './auth';
              ...
              return this.authService.fetchOrRegisterUser(request).pipe(
                ...
                catchError(err => {
                  ...
                  this.toastController.create({
                    animated: true,
                    message: "Login with Google failed. Please try agian.",
                    duration: 3000,
                    position: "middle",
                  }).then(toast => {
                    toast.present();
