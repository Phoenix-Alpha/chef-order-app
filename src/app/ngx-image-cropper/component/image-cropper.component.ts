...
export class ImageCropperComponent implements OnChanges, OnInit {
  ...
  private checkImageMaxSizeRecursively(): void {
    if (this.setImageMaxSizeRetries > 40) {
      ...
      this.setCropperScaledMaxSize();
      this.resetCropperPosition();
      // bug fix for white gap      
      this.renderer.setStyle(this.wrapper.nativeElement, "height", this.maxSize.height.toString() + "px", RendererStyleFlags2.Important);
  ...
  private sourceImageLoaded(): boolean {
    return this.sourceImage && this.sourceImage.nativeElement && this.sourceImage.nativeElement.width > 0;
  }
  ...
  private resizeCropperPosition(): void {
    ...
    if (this.maxSize.width !== sourceImageElement.width || this.maxSize.height !== sourceImageElement.height) {
      this.cropper.x1 = this.cropper.x1 * sourceImageElement.width / this.maxSize.width;
      this.cropper.x2 = this.cropper.x2 * sourceImageElement.width / this.maxSize.width;
      this.cropper.y1 = this.cropper.y1 * sourceImageElement.height / this.maxSize.height;
      this.cropper.y2 = this.cropper.y2 * sourceImageElement.height / this.maxSize.height;
    }
  }
  ...
  private setMaxSize(): void {
    if (this.sourceImage) {
      const sourceImageElement = this.sourceImage.nativeElement;
      this.maxSize.width = sourceImageElement.width;
      this.maxSize.height = sourceImageElement.height;
